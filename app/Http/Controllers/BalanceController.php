...
class BalanceController extends Controller
{
    ...
    public function balance_my(Request $request)
    {
        ...
        $balance = Auth::user()->balance->where('description','USD Wallet')->first();
        if($balance){
    ...
    public function balance_harvest(Request $request)
    {
        ...
        $balance = Auth::user()->balance->where('description','Trustme Coin')->first();
        if($balance){
            $data = HistoryTransaction::where('balance_id',$balance->id)
                ->whereDate('created_at','>=',$from)
                ->whereDate('created_at','<=',$to)
        ...
        $id = null;
        return view('backend.wallet.dinasty_coin', compact('data','total','from_date','to_date','id','saldo'))->with('i', (request()->input('page', 1) - 1) * 20);
    }
    ...
    public function balance_my_member(Request $request,$id)
    {
        ...
        $balance = Balance::where(['user_id'=>$id,'description'=>'USD Wallet'])->first();
        if($balance){
            $data = HistoryTransaction::where('balance_id',$balance->id)
                ->whereDate('created_at','>=',$from)
                ->whereDate('created_at','<=',$to)
        ...
        return view('backend.wallet.my_wallet', compact('data','total','from_date','to_date','id','saldo','balance'))->with('i', (request()->input('page', 1) - 1) * 20);
    }
    ...
    public function balance_harvest_member(Request $request,$id)
    {
        ...
        $balance = Balance::where(['user_id'=>$id,'description'=>'Trustme Coin'])->first();
        if($balance){
            $data = HistoryTransaction::where('balance_id',$balance->id)
                ->whereDate('created_at','>=',$from)
                ->whereDate('created_at','<=',$to)
        ...
        return view('backend.wallet.dinasty_coin', compact('data','total','from_date','to_date','id','saldo','balance'))->with('i', (request()->input('page', 1) - 1) * 20);
    }
